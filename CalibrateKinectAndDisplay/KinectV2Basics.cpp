...
bool KinectV2Basics::GetDepthMat(Mat& outDepth16S)
{
	...
	if (SUCCEEDED(hResult))
	{
		...
		cout << bufferMat.size().width << ", " << bufferMat.size().height << endl;
		// y軸に対して反転...しない！
		Mat bufferMatFlip = bufferMat;
...
bool KinectV2Basics::GetInfraredMat(cv::Mat& outInfrared)
{
	...
	outInfrared = Mat::zeros(heightInfrared, widthInfrared, CV_8UC4);
	Mat bufferMat(heightDepth, widthDepth, CV_16SC1);
		...
		if (SUCCEEDED(hResult))
		{
			for (int y = 0; y < heightInfrared; ++y)
			{
				for (int x = 0; x < widthInfrared; ++x)
				{
					...
					UCHAR* dataInfrared = &outInfrared.data[index];
			...
			//Mat bufferMat2;
			//bufferMat.convertTo(bufferMat2, CV_8U, 255.0f / 65535.0f, 0.0f);
			//imshow("teset", outInfrared);
	/*		Mat infraredMat(heightInfrared, widthInfrared, CV_16UC1, &infraredBuffer[0]);
			imshow("teset", infraredMat);*/

			// カラー画像を格納
			//outInfrared = infraredMat;
